	...
	<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
		...
		<td class="td font-family text-align-<?php echo esc_attr( $price_text_align ); ?>" style="vertical-align:middle;">
			...
			if ($product_id) {
				...
				$bundled_product = get_field('bundled_product', $product_id);
				
				if ( (int) $bundled_product === 1 && ! $is_comp_order && ! $is_admin_order ) {
					$bundled_quantity = (int) get_field('bundled_quantity', $product_id);
					if ($bundled_quantity > 0) {
						$qty = $qty / $bundled_quantity;
						$refunded_qty = $refunded_qty / $bundled_quantity;
					}
				}

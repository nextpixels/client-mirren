				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<!--	<td class="product-remove">
						...
						if ( $_product->is_sold_individually() ) {
							...
							if ( get_field( 'bundled_product', $_product->id ) == '1' ) {
								$di_min_qty = 0;
							} else if ( in_array( $_product->id, [165, 166, 167, 177] ) ) {
								$di_min_qty = 0;
							} else {
								$di_min_qty = 1;
							}
...
//print_r($product_quantity);
			...
			<tr>
				<td colspan="6" class="actions">
					...
					<a href="<?php echo esc_url( add_query_arg( 'empty_cart', 'yes' ) ) ?>" class="" style="margin-right: 20px;">Empty cart</a>
					<button type="submit" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="update_cart" value="<?php esc_attr_e( 'Update Registration', 'woocommerce' ); ?>"><?php esc_html_e( 'Update Registration', 'woocommerce' ); ?></button>

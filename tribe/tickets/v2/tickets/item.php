...
// Use the same ACF-first price as the cart. Event Tickets multiplies this
// per-attendee value by the raw quantity stored in the native input.
$ticket_pricing    = di_get_ticket_pricing( $ticket->ID, $ticket->price );
$ticket_unit_price = $ticket_pricing
	? $ticket_pricing['unit_price']
	: $ticket->price;

// Addon Ticket Parent ID
$addon_parent = get_field('parent_id', $ticket->ID) ?: '';
...
$attributes = [
	...
	'data-ticket-price'   => (string) $ticket_unit_price,
	'data-parent-id'	  => $addon_parent,
];

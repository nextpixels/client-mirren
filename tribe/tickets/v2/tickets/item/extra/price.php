...
// Use the same ACF-first price as the cart and checkout.
$ticket_pricing = di_get_ticket_pricing( $ticket->ID, $ticket->price );
$price_to_show  = $ticket_pricing
	? $ticket_pricing['display_price']
	: $ticket->price;
...
if (
	$ticket_pricing &&
	'sale_price' === $ticket_pricing['source'] &&
	false !== $ticket_pricing['acf_normal_price']
) {
	$original_price = $ticket_pricing['acf_normal_price'];
} elseif (
	$ticket_pricing &&
	'default' === $ticket_pricing['source'] &&
	! empty( $ticket->on_sale )
) {
	$native_regular_price = di_normalize_ticket_price(
		$ticket->regular_price
	);

	if ( false !== $native_regular_price ) {
		$original_price =
			$native_regular_price *
			$ticket_pricing['quantity_step'];
	}
}
?>
<div <?php tribe_classes( $classes ); ?>>
	<?php /*if ( false !== $original_price ) : ?>
		<span class="tribe-common-b2 tribe-tickets__tickets-original-price">
			<?php echo $currency->get_formatted_currency_with_symbol( $original_price, $post_id, $provider->class_name ); ?>
		</span>
	<?php endif;*/ ?>
	<span class="tribe-tickets__tickets-sale-price">
		<?php echo $currency->get_formatted_currency_with_symbol( $price_to_show, $post_id, $provider->class_name ); ?>
		<?php if ( $has_suffix ) : ?>

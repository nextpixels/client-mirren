...
$is_bundled = (bool) get_field(
	'bundled_product',
	$ticket->ID
);

$bundled_quantity = $is_bundled
	? absint( get_field( 'bundled_quantity', $ticket->ID ) )
	: 1;

// Use the normal single-ticket increment if the ACF value is missing.
$quantity_step = max( 1, $bundled_quantity );
...
<div <?php tribe_classes( $classes ); ?>>
	...
	<?php if ( $is_bundled ) : ?>
		<div
			id="item-quantity-number--<?php echo esc_attr( absint( $ticket->ID ) ); ?>"
			class="tribe-common-h3 tribe-common-h4--min-medium tribe-tickets__tickets-item-quantity-number-input qty-bundled-input"
		>
			0
		</div>
	<?php endif; ?>
